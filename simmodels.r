...
#		 i.e. (x0, x0+1, x0+2, ...)
    ...
    list(y=y[n]+(1:tmax)-1,xx=xx,xv=xv[1:n1],mu2=mu2,v22=v22,dda=dda,pa=pa,tpa=tpa,qaa=qaa)
}
...
#		 i.e. (x0, x0+1, x0+2, ...)
    ...
    {   # nsim is the number of sample paths that
	  ...
	  for(i in 1:lxx)
        ...
        list(y=y[n]+(1:tmax)-1,xx=xx,xv=xv,muh=muh,vh=vh,dda=dda,pa=pa,tpa=tpa,qaa=qaa)
}



fan=function(yv,mat,pl=0){
